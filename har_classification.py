...
features_df = pd.read_csv('UCI HAR Dataset/features.txt', sep=r'\s+', header=None, names=['idx', 'feature_name'])
activity_df = pd.read_csv('UCI HAR Dataset/activity_labels.txt', sep=r'\s+', header=None, names=['idx', 'activity'])
...
y_train = pd.read_csv('UCI HAR Dataset/train/y_train.txt', sep=r'\s+', header=None).values.ravel()
X_test = pd.read_csv('UCI HAR Dataset/test/X_test.txt', sep=r'\s+', header=None)
y_test = pd.read_csv('UCI HAR Dataset/test/y_test.txt', sep=r'\s+', header=None).values.ravel()
...
print(f"Original features: {X_train_scaled.shape[1]} -> Reduced: {X_train_pca.shape[1]} (95% variance)")
...
print(f"  Accuracy  - Train: {accuracy_score(y_train, y_train_pred_sgd):.4f}, Test: {accuracy_score(y_test, y_test_pred_sgd):.4f}")
print(f"  Loss      - Train: {log_loss(y_train, y_train_proba_sgd):.4f}, Test: {log_loss(y_test, y_test_proba_sgd):.4f}")
print(f"  Precision - Train: {precision_score(y_train, y_train_pred_sgd, average='weighted'):.4f}, Test: {precision_score(y_test, y_test_pred_sgd, average='weighted'):.4f}")
...
for depth in depths:
    ...
    print(f"  max_depth={str(depth):>4} -> Train Acc: {train_acc:.4f}, Test Acc: {test_acc:.4f}")
...
print(f"  Accuracy  - Train: {accuracy_score(y_train, y_train_pred_rf):.4f}, Test: {accuracy_score(y_test, y_test_pred_rf):.4f}")
print(f"  Loss      - Train: {log_loss(y_train, y_train_proba_rf):.4f}, Test: {log_loss(y_test, y_test_proba_rf):.4f}")
print(f"  Precision - Train: {precision_score(y_train, y_train_pred_rf, average='weighted'):.4f}, Test: {precision_score(y_test, y_test_pred_rf, average='weighted'):.4f}")
